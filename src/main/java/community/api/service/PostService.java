...
public class PostService {
    ...
    public PostResponseDto createPost(Long userId, PostRequestDto request) {
        ...
        List<String> tagNames = validateTags(request.getTags());
    ...
    public List<PostResponseDto> getPosts(Long userId, PostSearchDto search) {
        if (userId == null) {
            throw new UnauthorizedException("unauthorized_error");
        }
        ...
        return posts.stream()
                .map(post -> {
                    ...
                    boolean isLiked =
                            likeRepository.existsByPost_IdAndUser_Id(
                                    post.getId(),
                                    userId
                            );
    ...
    public PostResponseDto updatePost(
        ...
        if (userId == null) {throw new UnauthorizedException("unauthorized_error");}

        Post post = postRepository.findByIdAndDeletedAtIsNull(postId)
                .orElseThrow(() -> new NotFoundException("post_not_found"));

        if (!post.getUserId().equals(userId)) {throw new ForbiddenException("forbidden_error");}
    ...
    private PostResponseDto toResponseDto(Post post, Long currentUserId) {
        ...
        int commentCount = (int) commentRepository.countByPost_IdAndDeletedAtIsNull(post.getId());
        boolean isLiked =
                currentUserId != null &&
                        likeRepository.existsByPost_IdAndUser_Id(
                                post.getId(),
    ...
    private List<String> validateTags(List<String> tags) {
        ...
        if (new HashSet<>(normalizedTags).size() != normalizedTags.size()) {
